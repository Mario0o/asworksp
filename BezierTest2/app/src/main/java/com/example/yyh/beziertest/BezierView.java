package com.example.yyh.beziertest;
...
public class BezierView extends FrameLayout {
    ...
    private final ThreadLocal<Paint> paint = new ThreadLocal<>();
    private Path path;
    ...
    private void init(){
        ...
        paint.set(new Paint());
        paint.get().setAntiAlias(true);
        paint.get().setStyle(Paint.Style.FILL_AND_STROKE);
        paint.get().setStrokeWidth(2);
        paint.get().setColor(Color.RED);
    ...
    protected void onDraw(Canvas canvas){
        if(isAnimStart || !isTouch){
            ...
            calculate();
            canvas.drawColor(Color.TRANSPARENT, PorterDuff.Mode.OVERLAY);
            canvas.drawPath(path, paint.get());
            canvas.drawCircle(startX, startY, radius, paint.get());
            canvas.drawCircle(x, y, radius, paint.get());
        }
        super.onDraw(canvas);
    }
